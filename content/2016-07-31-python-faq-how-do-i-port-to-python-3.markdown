...
If you're using the `pickle` module (which you [shouldn't be]({filename}/updates/2015-10-15-dont-use-pickle-use-camel.markdown)), and you intend to pass pickles back and forth between Python 2 and Python 3, there's a small issue to be aware of.  `pickle` has several different ["protocol" versions](https://docs.python.org/3/library/pickle.html#pickle-protocols), and the default version used in Python 3 is protocol 3, which Python 2 _cannot read_.
